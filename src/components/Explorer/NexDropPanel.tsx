...
import { Wifi, Send, Smartphone, Monitor, Laptop, Tablet, RefreshCw, Shield, Globe, CheckCircle2, Server, FolderInput, QrCode, Trash2, File } from 'lucide-react';
...
const NexDropPanel: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'nexdrop' | 'server'>('nexdrop');
  ...
  useEffect(() => {
    ...
    const loadAndStartReceiver = async () => {
      try {
        ...
        console.log('NexDrop file receiver listener started in:', path);
      } catch (e) {
        console.error('Failed to get download dir or start receiver:', e);
        setDefaultUsbPath('Downloads');
      }
    };
    loadAndStartReceiver();
  ...
  const startScanning = async () => {
    ...
    setDevices([]);
    try {
      await invoke('start_nexdrop_discovery');
      await invoke('send_nexdrop_announcement');
    } catch (e) {
  ...
  const sendToDevice = async (device: Device | TrustedDevice) => {
    try {
      let pathsToSend = queuedPaths;

      // If there are no queued paths, fall back to file selector dialog
      if (pathsToSend.length === 0) {
        ...
        if (selected) {
          pathsToSend = selected;
        }
      }

      if (pathsToSend && pathsToSend.length > 0) {
        for (const path of pathsToSend) {
          await invoke('send_file_via_nexdrop', {
            filePath: path,
            targetIp: device.ip,
            targetPort: device.port
          });
        }
        // If we successfully sent context queued files, clear the queue
      ...
      >
        <div className="flex flex-col items-center text-center mb-8">
          ...
          <AnimatePresence>
            {showQR && (
              ...
              >
                <QRCodeSVG value={`nexdrop://connect`} size={150} />
                <p className="text-[10px] text-black mt-2 font-bold uppercase tracking-wider">Scan to Connect</p>
              </motion.div>
            )}
          </AnimatePresence>

          <h1 className="text-3xl font-bold tracking-tight text-text-primary mb-3">NexDrop</h1>

          {/* Tab Switcher */}
          <div className="flex bg-bg-tertiary p-1 rounded-2xl border border-border/50 mt-4">
            <button
              onClick={() => setActiveTab('nexdrop')}
              className={`flex items-center gap-2 px-6 py-2 rounded-xl text-xs font-bold transition-all ${activeTab === 'nexdrop'
                  ? 'bg-accent text-white shadow-lg shadow-accent/20'
                  : 'text-text-muted hover:text-text-primary'
                }`}
            >
              <Wifi size={14} /> NEXDROP
            </button>
            <button
              onClick={() => setActiveTab('server')}
              className={`flex items-center gap-2 px-6 py-2 rounded-xl text-xs font-bold transition-all ${activeTab === 'server'
                  ? 'bg-accent text-white shadow-lg shadow-accent/20'
                  : 'text-text-muted hover:text-text-primary'
                }`}
            >
        ...
        <AnimatePresence mode="wait">
          {activeTab === 'nexdrop' ? (
            ...
            >
              {/* Nearby Devices & Queued Files */}
              <div className="lg:col-span-2 space-y-6">

                {/* Context-selected Queued Files Card */}
                <AnimatePresence>
                  {queuedPaths.length > 0 && (
                    <motion.div
                      ...
                      className="glass-card rounded-2xl p-5 border-accent/40 bg-accent/[0.02] shadow-sm"
                    >
                      <div className="flex items-center justify-between mb-3 border-b border-border/40 pb-2">
                        <h4 className="text-xs font-bold text-accent uppercase tracking-wider flex items-center gap-2">
                          <File size={14} /> Queued Files ({queuedPaths.length})
                        </h4>
                        <button
                          onClick={clearQueuedPaths}
                          className="flex items-center gap-1 text-[10px] font-bold text-text-muted hover:text-error transition-colors"
                        >
                      ...
                      <div className="max-h-24 overflow-y-auto custom-scrollbar space-y-1.5 pr-1">
                        {queuedPaths.map((path) => (
                          <div key={path} className="flex items-center justify-between bg-bg-secondary/40 p-2 rounded-lg border border-border/30 text-xs">
                            <span className="font-mono text-text-primary truncate flex-1 min-w-0" title={path}>
                              {getBasename(path)}
                            </span>
                            <button
                              onClick={() => removeQueuedPath(path)}
                              className="text-text-muted hover:text-error transition-colors p-0.5 flex-shrink-0 ml-2"
                            >
                              &times;
                            </button>
                          </div>
                      ...
                      <p className="text-[10px] text-text-muted mt-2">
                        Select an online device or folder below to send these queued files.
                      </p>
                    </motion.div>
                  )}
                </AnimatePresence>
                  ...
                  <div className="flex items-center justify-between px-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[11px] font-bold transition-all ${isScanning
                    ...
                    <AnimatePresence mode="popLayout">
                      ...
                      >
                        <div className="flex items-center justify-between mb-3">
                          <div className="flex items-center gap-3">
                            ...
                            <div className="text-left w-full overflow-hidden">
                              ...
                              <div className="text-[10px] font-mono text-text-muted opacity-80 mt-0.5 truncate max-w-[180px]" title={defaultUsbPath}>{defaultUsbPath}</div>
                            </div>
                          </div>
                        ...
                        <div className="flex gap-2 w-full mt-auto pt-2 border-t border-accent/20">
                          ...
                          >
                            <Send size={10} /> {queuedPaths.length > 0 ? 'DROP QUEUE' : 'SEND'}
                          </button>
                          <button
                        ...
                        [...new Map([...trustedDevices, ...devices].map(item => [item.ip, item])).values()].map((device, idx) => {
                          ...
                          return (
                            <motion.div
                              ...
                              className={`group flex flex-col p-4 glass-card rounded-2xl border-border/50 transition-all ${isLive ? 'hover:bg-accent/5 hover:border-accent/40' : 'opacity-60 grayscale hover:grayscale-0 hover:opacity-100'
                                }`}
                            >
                              <div className="flex items-center justify-between mb-3">
                                <div className="flex items-center gap-3">
                                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center shadow-inner transition-colors ${isLive ? 'bg-bg-tertiary text-accent group-hover:bg-accent group-hover:text-white' : 'bg-bg-secondary text-text-muted'
                                    }`}>
              ...
              <div className="space-y-6">
                ...
                <div className="glass-card rounded-3xl p-6 relative overflow-hidden group">
                  ...
                  <div className="flex items-center gap-4 mb-8">
                    ...
                    <div>
                      <div className="font-bold text-[15px] text-text-primary">NexDrop</div>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between p-3 bg-bg-primary/40 rounded-xl border border-border/30">
                      ...
                      <button
                        ...
                        className={`relative w-10 h-5 rounded-full transition-colors duration-200 ${isDiscoverable ? 'bg-accent shadow-lg shadow-accent/20' : 'bg-bg-tertiary border border-border'
                          }`}
                      >
                        <div className={`absolute top-0.5 w-4 h-4 rounded-full bg-white transition-transform duration-200 ${isDiscoverable ? 'translate-x-5' : 'translate-x-0.5'
                          }`} />
                      </button>
                    </div>
                    ...
                    <div className="flex items-center justify-between p-3 bg-bg-primary/40 rounded-xl border border-border/30">
                      ...
                      <button
                        ...
                        className={`relative w-10 h-5 rounded-full transition-colors duration-200 ${isAutoAccept ? 'bg-success shadow-lg shadow-success/20' : 'bg-bg-tertiary border border-border'
                          }`}
                      >
                        <div className={`absolute top-0.5 w-4 h-4 rounded-full bg-white transition-transform duration-200 ${isAutoAccept ? 'translate-x-5' : 'translate-x-0.5'
                          }`} />
                      </button>
                    </div>
                  </div>
