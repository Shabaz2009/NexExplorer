...
// B36 fix: generate a stable ID for the initial tab so activeTabId is never ''.
const INITIAL_TAB_ID = 'initial-tab';

export const useTabStore = create<TabStore>()(
  // B35 fix: persist tabs across restarts.
  persist(
    (set) => ({
      tabs: [
        {
    ...
    {
      name: 'nex-tabs',
      // Only persist tab data, not function references.
      partialize: (state) => ({
        tabs: state.tabs,
