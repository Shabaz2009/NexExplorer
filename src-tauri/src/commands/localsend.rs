...
/// LocalSend Protocol 2.1 Constants
/// Ported from localsend-main/common/lib/constants.dart
pub const MULTICAST_IP: &str = "224.0.0.167";
pub const PORT: u16 = 53317;
pub const PROTOCOL_VERSION: &str = "2.1";

/// Multicast DTO — directly mirrors localsend-main/common/lib/model/dto/multicast_dto.dart
...
/// Device representation — mirrors localsend-main/common/lib/model/device.dart
#[derive(Debug, Serialize, Deserialize, Clone)]
pub struct Device {
    pub ip: String,
    pub version: String,
...
/// Ported from localsend-main/common/lib/src/task/discovery/multicast_discovery.dart
...
#[tauri::command]
pub async fn start_discovery(app: tauri::AppHandle) -> Result<(), String> {
    let socket = match UdpSocket::bind(("0.0.0.0", PORT)) {
        Ok(s) => s,
        Err(e) => return Err(format!("Failed to bind to UDP port {}: {}", PORT, e)),
    };
...
/// Mirrors localsend-main/common/lib/src/task/discovery/multicast_discovery.dart#sendAnnouncement
/// Sends 3 announcements with increasing delays (100ms, 500ms, 2000ms) for reliability.
#[tauri::command]
pub async fn send_multicast_announcement() -> Result<(), String> {
    ...
    // Send 3 times with increasing delays (mirrors LocalSend behavior)
...
/// Ported from localsend-main/common/lib/src/task/upload/http_upload.dart
/// Listens on the same port for incoming file transfers via HTTP POST.
#[tauri::command]
pub async fn start_file_receiver(
    app: tauri::AppHandle,
    save_dir: String,
) -> Result<(), String> {
    let listener = TcpListener::bind(format!("0.0.0.0:{}", PORT + 1))
        .map_err(|e| format!("Failed to bind TCP: {}", e))?;
    
    let save_path = save_dir.clone();
    let app_clone = app.clone();
    ...
    thread::spawn(move || {
        for stream in listener.incoming() {
            if let Ok(stream) = stream {
                ...
                thread::spawn(move || {
                    if let Err(e) = handle_file_receive(stream, &save_path, &app_inner) {
...
fn handle_file_receive(
    ...
    loop {
        ...
        let progress = TransferProgress {
            ...
            percent: (total_read as f64 / info.file_size as f64) * 100.0,
        };
...
/// Mirrors localsend-main/common/lib/src/task/upload/http_upload.dart#upload
#[tauri::command]
pub async fn send_file_to_device(
    ...
    loop {
        ...
        let progress = TransferProgress {
            ...
            percent: (total_sent as f64 / file_size as f64) * 100.0,
        };
...
fn get_device_fingerprint() -> String {
    ...
    // Simple hash — in production, use a proper machine ID
